...
3. **Start development server**
   ...
   - **Start a public tunnel (separate terminal):**
      ```bash
      npm run tunnel
      ```

   - **Notes:**
      - This uses `npx ngrok` so you don't need a global install. To use the official ngrok client long-term, install it and add your authtoken:
         ```bash
         npx ngrok authtoken YOUR_AUTHTOKEN
         ```
      - Run the dev server and the tunnel in separate terminals. If you prefer one command, install `concurrently` and add a combined script.
